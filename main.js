        ...
        /* --- App-Status & globale Variablen (unverändert) --- */
        ...
        function formatCoordsListHTML(list, basePriority, nextMove, title) {
            ...
            if (list.length > 5) {
                ...
                html += `, <button class="show-full-list underline text-sm"
                        data-list-enc="${payload}" data-title="${title}">
                        ... (${list.length - 5} weitere)
                    </button>`;
            }
            return html;
        }
        ...
        function showListOverlay(listName) {
            const container =
                (zoomModal && zoomModal.style.display === 'flex')
                    ? gridContainerModal
                    : gridContainer;
            ...
            const nextMove = (currentData.move && currentData.move.length === 2)
                ? currentData.move
                : null;
        ...
        function clampWorldY(y) {
            return Math.max(WORLD_Y_MIN_CELL, Math.min(WORLD_Y_MAX_CELL, y));
        }
        ...
        function renderWorldVisualization() {
            ...
            if (selected.length && trailLayer) {
                selected.forEach(bid => {
                    ...
                    for (let i = 0; i < history.length; i++) {
                        ...
                        if (y < WORLD_Y_MIN_CELL || y > WORLD_Y_MAX_CELL) continue;
                        ...
                        const alpha = Math.max(0.2, 1 - (i * (0.8 / Math.max(1, N - 1))));
            ...
            // 3) Env-Projektionen & Sichtbarkeit (aktuelle Runde)
            if (selected.length && envLayer) {
                selected.forEach(bid => {
                    ...
                    for (let i = 0; i < env.length && i < 49; i++) {
                        ...
                        if (x < WORLD_X_MIN || x > WORLD_X_MAX ||
                            y < WORLD_Y_MIN_CELL || y > WORLD_Y_MAX_CELL) continue;
                        ...
                        // Map-Zellen, die gesehen werden -> discovered
                        ...
                        if (cell)
                            cell.style.background = 'var(--world-discovered)';
            ...
            // 4) Dots-Layer für vergangene Sichtfelder (N > 1)
            if (selected.length && dotsLayer) {
                for (let step = 1; step <= Math.max(0, N - 1); step++) {
                    const opacity = ((N - step) / N);

                    selected.forEach(bid => {
                        // Eintrag für (activeRound - step) finden (<= round)
                        ...
                        for (let i = 0; i < env.length && i < 49; i++) {
                            ...
                            dot.style.background = getBeastColor(bid);
        ...
        function initAppDom() {
            ...
            /* Keyboard Shortcuts (nur im Biester-Dashboard) */
            window.addEventListener('keydown', e => {
                ...
                if (currentDashboard !== 'beasterDashboard') return;
            ...
            /* Slider-Änderung -> Trails & Dots neu zeichnen */
